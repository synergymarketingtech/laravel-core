...
class TaskUserNotification extends Notification
{
     ...
     * @return void
     */
    public function __construct(Task $issue, Admin $user)
    {
        $this->issue = $issue;
        $this->user = $user;
        $issue->message = nl2br($issue->message);

        $this->message .= "<p>{$issue->message}</p>";

        if (count($issue->media)) {
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ...
            ->subject("[Task] {$this->issue->subject} by {$this->issue->user->name}")
            ->line(new HtmlString($this->message))
            ->action('Open Task', admin_url("tasks/{$this->issue->id}?action=edit"))
